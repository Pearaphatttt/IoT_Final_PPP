...
// ================== [เพิ่มใหม่] ตัวแปรควบคุม LED ด้วย count ==================
/*
  - นับเพิ่มทุกครั้งที่มีการเก็บตัวอย่าง (ทุก 5 วินาที)
  - ถ้า count % 2 == 0 -> LED_RED = LOW (OFF)
  - ถ้า count % 2 != 0 -> LED_RED = HIGH (ON)
  - ส่งค่า led ("ON"/"OFF") และ count ไป Firebase ทั้ง latest และ history
*/
unsigned long count = 0;                 // [เพิ่มใหม่] ตัวนับสำหรับ LED_RED
// ============================================================================
...
void setup() {
  ...
  // ================== [เพิ่มใหม่] กำหนดสถานะเริ่มต้นของ LED ตาม count ==================
  // เริ่มต้น count = 0 (ประกาศด้านบน) -> 0 % 2 == 0 ดังนั้น LED เริ่มเป็น LOW (OFF)
...
void loop() {
  ...
  // ================== [เพิ่มใหม่] นับรอบและควบคุม LED ตาม count % 2 ==================
  count++;                                    // เพิ่มตัวนับทุกครั้งที่เก็บตัวอย่าง
  bool ledOn = (count % 2 != 0);              // คำนวณสถานะ LED: คี่ = ON, คู่ = OFF
  digitalWrite(LED_RED, ledOn ? HIGH : LOW);  // สั่ง LED จริงตามสถานะ
  String ledStr = ledOn ? "ON" : "OFF";       // สร้างสตริงสำหรับบันทึกขึ้น Firebase
  // =====================================================================
  ...
  jsonLatest       += (comma?",":"") + String("\"lux\":")     + (isnan(lux)? String("null") : String(lux,1)); comma = true;  // lux

  // ------------------ [เพิ่มใหม่] ใส่ led และ count ลง latest ------------------
  jsonLatest       += (comma?",":"") + String("\"led\":\"") + ledStr + String("\""); comma = true;   // led: "ON"/"OFF"
  jsonLatest       += (comma?",":"") + String("\"count\":") + String(count);                         // count (ตัวเลข)
  // ---------------------------------------------------------------------------
  ...
  jsonHist         += (comma?",":"") + String("\"lux\":")      + (isnan(lux)? String("null") : String(lux,1)); comma = true; // lux

  // ------------------ [เพิ่มใหม่] ใส่ led และ count ลง history ------------------
  jsonHist         += (comma?",":"") + String("\"led\":\"") + ledStr + String("\""); comma = true;   // led: "ON"/"OFF"
  jsonHist         += (comma?",":"") + String("\"count\":") + String(count);                         // count
  // ---------------------------------------------------------------------------
